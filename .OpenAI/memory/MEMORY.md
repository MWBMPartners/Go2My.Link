...
2. **📝 Documentation** — Keep updated: README.md, CHANGELOG.md, PROJECT_STATUS.md, DEV_NOTES.md, the in-app Help section, and the API description (`openapi.yaml` → Redoc `/api/docs/` + Swagger UI `/api/docs/swagger/`)
3. **🧠 Claude Context** — Keep `.claude/` files current (this portable memory, ProjectBrief, plans, [HISTORY.md](../HISTORY.md))
4. **🤖 Codex Context** — Keep `.OpenAI/` current by running `sh scripts/sync-ai-context.sh` after editing `.claude/` or `CLAUDE.md` (a GitHub check fails if it is forgotten); update the root `AGENTS.md` by hand when a rule it summarises changes. *(Folder renamed from `.openai/` on 2026-09-21.)*
5. **🚫 .gitignore** — Dynamically maintain as new tools introduced
6. **🔗 PR Linking** — When closing issues, link the corresponding PR
7. **💾 Commit and push** — Commit **and push** each finished piece of work to the working branch (changed 2026-09-21; it used to be "commit, don't push"). One pull request to `alpha` later, when the owner says so — never stacked
