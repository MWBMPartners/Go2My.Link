...
> Codex gets the same context through the root [AGENTS.md](AGENTS.md) and `.OpenAI/` — a copy of
> this file and `.claude/memory/`, refreshed by `sh scripts/sync-ai-context.sh`. Edit the Claude
> side, then run that script.

## Read these first

- 🤝 **[.github/HANDOFF.md](.github/HANDOFF.md)** — live session pick-up point: what's done, in-flight, decisions pending, next steps. **Start here** — it is kept current every session; the docs below can lag it. It is the project's **one** handoff; never create a second, and never write it at the repository root (the root `HANDOFF.md` is the dev-team plugin's scratch card and is ignored by git). *(Moved from the repository root to `.claude/` on 2026-09-21, then to `.github/HANDOFF.md` on 2026-09-22 — owner decision 17, #243.)*
- 🧭 **[.claude/memory/working-rules.md](.claude/memory/working-rules.md)** — the **standing working rules** (plain English, handoff kept current as you go, how to plan/build/review, cross-system review until clean, steps after each task, documentation sweep, one branch and one pull request, what to do when an AI service runs out). Read before starting work.
...
- **State (2026-07-19):** A + B are code-complete for launch. **C (LinksPage) is now BUILT** (6/6 issues #45–#50 — renderer, custom-domain fallback, templates, management UI, sanitised custom HTML, age gate); `customHTML`/WYSIWYG is premium-gated with a kill switch defaulting OFF pending security sign-off. Also built: public API v1 (framework + endpoints + key management + OpenAPI/Redoc docs), analytics + dashboard + CSV export, IP geolocation (gated off), UTM capture/forward (gated off), custom domains with DNS-TXT verification, premium-tier entitlement gating, CueRCode integration path. Remaining work is owner-blocked (credential rotation, DB cutover, legal sign-off, SIGNula/billing credentials) — see **`.github/HANDOFF.md`** for the authoritative live state.
...
- 💾 **Commit and push each finished piece of work to the working branch** (the one branch that will later reach `alpha` through a single pull request, opened when the owner says so). *(Changed 2026-09-21 by the owner — it used to be "commit, don't push".)* Never push directly to `alpha`/`beta`/`release-candidate`/`main` without an explicit instruction; never force-push, hard-reset or change a remote without an explicit instruction, and never run any other destructive git operation (deleting branches, discarding uncommitted work, rewriting history) without an explicit go-ahead. 🔒 Never write/commit real credentials.
- 👤 **Never write the owner's real name** — use the GitHub username `Salem874` everywhere.
- 🧠 **Deep analysis and planning: Opus, one agent at a time** (changed 2026-09-23 — it used to be Fable first). Building: Sonnet or Haiku, whichever fits; Opus when genuinely complex. Checking never uses a weaker model than the building.
- 🔁 **Every change is reviewed by a different AI system from the one that built it** (Claude Code's work by Codex, and the other way round), round after round until a review comes back clean. See [working-rules.md](.claude/memory/working-rules.md) rules 6 and 13.
