...
> "link in bio" services. The finished pieces are listed under START HERE; the next are SX-205,
> SX-212 and PF-13 (the rest of batch 5). 734 unit tests and 234 database tests pass. The programme covers: making every tier feature editable in the
...
## ▶️ START HERE — everything a fresh session needs (2026-09-25, 15:05)
...
(`b59ef73`), SX-147 (`3562029`), SX-214 (`8402425`) and SX-206 (`a173c8a`) — batch 4 is complete — and
SX-208 (`a37dfd1`) from batch 5, plus commits to the build tools and these notes (`9669b25`,
...
**Next in this session:** SX-205 and SX-212, then PF-13 (the rest of batch 5).

**The second Codex catch-up ran at 14:44–14:46 and came back clean:** it reviewed `3562029`,
`8402425`, `b59ef73`, `ade7c02` and `cbb8a4d` one by one and found no defect and no house-rule
breach. **SX-208's second review round was done by Codex itself** (clean), the first in-line Codex
review of the session. Every issue whose commit Codex has now passed carries a comment saying so
(14 issues across both catch-ups). The review copy of the repository is removed and the lock file
is gone, so item reviewers try Codex first again.
...
so. The finished pieces are listed below. The next items are **SX-205**, **SX-212** and **PF-13** (the
rest of batch 5); `progress.next` in `.claude/programme/build-plan.json` says the same.
...
| `1637f9d` | — | Notes: SX-147 and SX-214 recorded; second Codex catch-up scheduled. |
| `a173c8a` | #206 | An API key whose stored expiry date could not be read was being let through as if it never expired; both the API sign-in check and the admin API-keys page now treat an unreadable expiry as expired, using one shared function so the two places can no longer drift apart. SX-206. Review: 1 round(s); reviewers by round: claude-opus-fallback; last round clean. |
| `a37dfd1` | #208 | Removed the `domains:read` and `domains:write` API scopes, which could be granted on a key but were never checked by any route or handler; the API-keys page, the API docs and the in-app help no longer offer them, and a new test proves every remaining scope is actually enforced somewhere. SX-208. Review: 2 round(s); reviewers by round: claude-opus-fallback, codex; last round clean. |
| (this commit) | #205 | Editing a link on the dashboard or through the API used to run fewer checks than creating one, so an existing link could be edited to point at an internal address or back at one of our own short domains — destinations creating it would have refused; all three checks now live in one shared function that create, dashboard edit and the API update all call. SX-205. Review: 4 round(s); reviewers by round: claude-opus-fallback, claude-opus-fallback, claude-opus-fallback, claude-opus-fallback; last round clean. |

**Test baseline now: 734 unit tests and 234 database tests, all passing** (PHP 8.4 in Docker; MySQL
8.4 with `utf8mb4_unicode_ci`). Run them with `sh .claude/programme/run-tests.sh all`.

### ⏭️ Next actions, in order

1. **The rest of batch 5:** SX-205 (#205 — check a link's destination), SX-212 (#212 — form rate
   limits), PF-13 (personal plans; the foundation for decisions 25 and 26).
2. Then batch 6 onwards (see `build_order` in the build plan).
...
- **Owed a review:** the product code up to `272e69d` was reviewed at 09:42 on 2026-09-25 (see "This
  ...
  14:43. **Update 15:05:** the second catch-up (14:44) passed all the product code up to `8402425`,
  and Codex reviewed SX-208's final round itself. What is still owed is only the notes and
  build-tool commits, and SX-206 (`a173c8a`). Run it in a separate
