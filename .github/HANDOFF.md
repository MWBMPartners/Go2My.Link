...
| (this commit) | #204 | The info page now accepts hyphens and underscores in a short code instead of silently stripping them, so a custom code such as "spring-sale" looks up that link instead of a different one (or none). SX-204. Review: 1 round(s); reviewers by round: claude-opus-fallback; last round clean. |
