...
| **GT-6** | ✅ **RESOLVED 2026-07-10.** The **two "domain" tables are disconnected.** `tblOrgShortDomains` (unverified, drives routing) vs `tblOrgDomains` (DNS-TXT-verified via real `verifyDomain()`, but nothing routes off it). The `org.max_short_domains` quota setting is **dead code** — never read. *(Both sub-claims were already independently fixed by the time this assessment ran: `org.max_short_domains` was wired into `addOrgShortDomain()` by #146, and the domain-table disconnect was resolved not by unifying the tables but by formally DEPRECATING `tblOrgDomains` — it was confirmed genuinely unused for routing, its admin page no longer accepts new rows, and `tblOrgShortDomains` remains the single routing/verification/LinksPage-designation table. See `web/_sql/migrations/018_deprecate_org_domains.sql` and `docs/CUSTOM_DOMAINS.md`'s roadmap section.)* | `domain_resolver.php`, `sp_lookupShortURL.sql`, `org.php:1002` `addOrgShortDomain` (no quota/verification). | Central design constraint for §5 (#91). |
